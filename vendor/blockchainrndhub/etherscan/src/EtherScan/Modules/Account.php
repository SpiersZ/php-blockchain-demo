...
class Account extends AbstractHttpResource
{
     ...
     * @return string
     */
    public function getTokenBalance(string $address, $contractaddress): string
    {
        $url = $this->getTokenBalanceLink($address, $contractaddress);
        return $this->apiConnector->doRequest($url);
    }
     ...
     * @param string $address
     */
    public function getTokenBalanceLink(string $address, string $contractaddress)
    {
        if (!ctype_alnum($address)) {
            throw new InvalidArgumentException('Argument address is invalid');
        }
     ...
     * @return string
     */
    public function getTokenTransactions(string $address, string $contractaddress, int $page, int $pageSize, string $sort): string
    {
        $url = $this->getTokenTransactionsLink($address, $contractaddress, $page, $pageSize, $sort);
        return $this->apiConnector->doRequest($url);
    }
     ...
     * @throws InvalidArgumentException
     */
    public function getTokenTransactionsLink(string $address, string $contractaddress, int $page = 1, int $pageSize = 25, string $sort =
    Account::SORT_DESC)
    {
        if (!ctype_alnum($address)) {
            throw new InvalidArgumentException('Argument address is invalid');
